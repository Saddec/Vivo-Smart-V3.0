...
void AudioManager::begin() {
    SPI.begin(18, 19, 23);
    if (!SD.begin(SD_CS)) Serial.println("[Audio] SD failed!");
    else Serial.println("[Audio] SD OK.");
    _audio = new Audio();
...
void AudioManager::stop() {
    if (_audio) { _audio->stopSong(); _state = AUDIO_IDLE; _currentPriority = 0; _customDuration = 0; _loopDuration = 0; _lastPlayedFile = ""; }
}
...
AudioState AudioManager::getState() {
    if (_state == AUDIO_PLAYING && !_audio->isRunning()) {
        audioOnStop(this); // سيتعامل مع loop
    }
